...
# def async_handle_songpallib_errors(
#     func: Callable[Concatenate[_DeviceT, _P], Awaitable[ucapi.StatusCodes | None]],
# ) -> Callable[Concatenate[_DeviceT, _P], Coroutine[Any, Any, ucapi.StatusCodes | None]]:
#     """Log errors occurred when calling a Sony AVR receiver.
#
#     Decorates methods of Sony Device class.
#
#     Taken from Home-Assistant
#     """
#
#     @wraps(func)
#     async def wrapper(self: _DeviceT, *args: _P.args, **kwargs: _P.kwargs) -> ucapi.StatusCodes:
...
class SonyDevice:
    ...
    async def async_activate_websocket(self):
        ...
        async def _volume_changed(volume: VolumeChange):
            ...
            self._is_volume_muted = volume.mute
        ...
        async def _source_changed(content: ContentChange):
            ...
            if content.is_input:
                ...
                _LOG.debug("Sony AVR Got non-handled content change: %s", content)
        ...
        async def _try_reconnect(connect: ConnectChange):
            ...
            _LOG.warning(
                "Sony AVR [%s(%s)] Connection reestablished", self._name, self._receiver.endpoint
            )
    ...
    #         discovered = await discover.sony_avrs()
    ...
    #
    # @async_handle_songpallib_errors
